...
### ✅ Sprint 15: scalar trigonometry + IPv4 family + parse_command_line + covariance — COMPLETE (2026-08-17)
...
6. ✅ **`parse_user_agent` deliberately deferred, not attempted** — real ADX's own implementation is "built on regex checks... against a huge number of predefined patterns" (the `ua-parser` project's own regex database), and no worked example with real output field values was found to verify against. Same category of decision as this project's own `reduce` scope note: an unverifiable regex reimplementation would be worse than an honest, documented gap. See `scalar_function_backlog.md`'s own `parse_user_agent` scope note for full reasoning.
7. ✅ **IPv6 family deliberately deferred, not attempted this sprint** — `ipv6_compare`, `ipv6_is_in_range`, `ipv6_is_in_any_range`, `ipv6_is_match`, `parse_ipv6`, `parse_ipv6_mask` remain open. A partial investigation found real ADX's own docs describe an embedded IPv4-notation prefix mapping onto the 128-bit IPv6 address space with a `96 + P` offset (confirmed directly from `parse_ipv6_mask`'s own worked example: an embedded `/24` on an IPv4-form address combines with an explicit 124-bit argument to yield an effective 120-bit mask, i.e. `96+24=120`), but a separate `ipv6_is_match` worked-example row mixing IPv4- and IPv6-notation arguments could not be reconciled with that same offset from the search-snippet fragments available this session — genuinely ambiguous without the full worked-output table, not a confident finding either way. Revisit with the full `ipv6-is-match-function.md` worked-example table fetched directly (not via search snippets) before implementing the mixed-notation comparison case; same-family (both-IPv4 or both-IPv6) cases are not blocked by this and could be implemented with confidence in a future session.
8. ✅ 25 new tests across four files (`func_trig_test.go`, `func_ipv4_test.go`, `func_parse_command_line_test.go`, `aggregation_covariance_test.go`) — one worked-example test per new function plus a dedicated regression test per bug found — full suite passes with `-race`, `go vet` clean, verified against all three real production scopes at every commit boundary.

Scalar functions: ~193 entry points implemented (was ~183). Aggregation functions: covariance family added (4 new).
...
7. **reduce** — see Sprint 12 item 5 and the "Out of Scope" section's own note above; revisit if the actual SLCT reference implementation (the `logparser` project's wrapped C source) or the full original paper text becomes directly readable.

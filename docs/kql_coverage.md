...
#### Network, Encoding & Hashing — `func_net.go` (34 functions)
...
### ✅ Sprint 15: scalar trigonometry + IPv4/IPv6 family + parse_command_line + covariance — COMPLETE (2026-08-17)
...
9. ✅ 33 new tests across five files (`func_trig_test.go`, `func_ipv4_test.go`, `func_parse_command_line_test.go`, `aggregation_covariance_test.go`, `func_ipv6_test.go`) — one worked-example test per new function plus a dedicated regression test per bug found — full suite passes with `-race`, `go vet` clean, verified against all three real production scopes at every commit boundary.

Scalar functions: ~199 entry points implemented (was ~183). Aggregation functions: covariance family added (4 new).

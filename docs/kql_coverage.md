...
Corrected 2026-08-14: `covariance`/`covarianceif`/`covariancep`/`covariancepif` were missing from this list entirely in the previous version — a genuine gap, found by cross-checking against a full real-ADX aggregation-function enumeration. The natural companion to the already-implemented `variance`/`variancep`/`varianceif`/`variancepif` family; see `scalar_function_backlog.md` for the full write-up.

| Function                    | Priority | Notes                            |
| ------------------------------ | -------- | ------------------------------------ |
| **covariance / covarianceif / covariancep / covariancepif** | 🟡 MED | Sample/population covariance between two columns, with conditional variants — same shape as the already-implemented variance family |
